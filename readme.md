...
* 2 Objects:
  ...
  - One that contains all useful information for every individual cell. Such information includes: opened (boolean), isMine (boolean), hasFlag (boolean), neighboring indexes (array), revealValue (string), revealNumber (number)
...
* Both a 'contextmenu' event and a 'click' event on each grid square. The 'contextmenu' event triggers the flagSquare function, and the 'click' event triggers the revealSquare function. Both of these events are set inside the createGrid function as soon as the individual cell is created rather than on page load because when the level is changed, grids are wiped away and reformed, so new event listeners will need to be called on the new cells.
...
* Winning and Losing
  - If a mine is selected, the revealSquare() method tells the loseGame function to fire.
  ...
  - If the number of opened cells equals the total cell count minus the number of mines, then the wonGame function fires
...
### Learning
---
There was quite a bit of learning in the nitty-gritty of coding. The biggest thing I learned was that it's important to organize JavaScript files in an intuitive way. Before the project I didn't envision how my code would be organized across different files, but on ensuing projects, this will be something I consider at the very beginning.
...
The other thing that could be done is to figure out how to make Minesweeper easier to play on a mobile device. At the moment, here and everywhere else, it's only viable on desktop due to the dimensions of the grid and how small the cells would have to be to all display in the same window on a mobile screen.